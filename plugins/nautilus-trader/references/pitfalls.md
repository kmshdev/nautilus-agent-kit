...
| Multiple run configs per `BacktestNode` | Snapshot allows at most one |
| Multi-config catalog streaming bounds all memory | Lazy merge avoids full input loading, not query buffers, equal-time expansion or retained engine state |
