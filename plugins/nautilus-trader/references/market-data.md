...
The snapshot permits at most one run config despite the vector constructor.
...
queries separately materialize `Vec<Data>`. Do not apply those eager-path
limitations to native multi-config streaming.
Chunks include all values sharing their boundary `ts_init`, so chunk size is
a target rather than a hard memory cap. Account for query buffers and retained
engine state too; measure representative peak memory. Run bounds intersect
data-config bounds.
