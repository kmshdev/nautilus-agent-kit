...
Initially published in kmshdev/plugins with `.codex-plugin/plugin.json` and named child
...
## Standalone repository — amended 2026-09-18

Maintain the extracted plugin in its own repository, retaining the existing
license and source provenance. The repository marketplace contains only
`plugins/nautilus-trader`; this wrapper is the plugin-creator install layout,
not an additional agent routing layer. Historical monorepo receipts remain
dated evidence, not qualification of the extracted revision.
