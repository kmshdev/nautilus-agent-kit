...
TOKEN_LIMIT_SELECTORS = {
    "inference/client.py": (
        ...
        '    if max_tokens is None:\n'
        '        return {}\n'
        '    return {"TOKEN_KEY": max_tokens}',
    ),
    "tier3/eval_core/llm_judge.py": (
        '    token_key = (\n'
...
TOKEN_LIMIT_SELECTORS["tier3/harbor/templates/eval.py"] = TOKEN_LIMIT_SELECTORS["tier3/eval_core/llm_judge.py"]
...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--completion-token-key", choices=("max_tokens", "max_completion_tokens"),
                        help="Explicit chat API token-limit key in the isolated evaluator; preserves numeric budgets")
...
def patch_source(text: str, old: str, new: str) -> str:
    if text.count(old) != 1 or new in text.replace(old, "", 1):
        raise ValueError("Evaluator compatibility patch does not match; no files were patched globally")
    return text.replace(old, new, 1)


def prepare_vendor(destination: Path, source: Path, completion_token_key: str | None = None) -> None:
    ...
    if completion_token_key is not None:
        if completion_token_key not in {"max_tokens", "max_completion_tokens"}:
            raise ValueError("Unsupported completion token key")
        for relative, (old, template) in TOKEN_LIMIT_SELECTORS.items():
            path = destination / "skillevaluator" / relative
            path.write_text(patch_source(path.read_text(), old, template.replace("TOKEN_KEY", completion_token_key)))
...
def complete_tier3(result: dict, count: int) -> bool:
    ...
    if not (
        ...
        return False
    if agent.get("execution_errors") != []:
        return False
    for key in ("agent_runtime_failures", "job_failures", "trial_failures"):
        failures = agent.get(key)
        expected_type = str if key == "job_failures" else list
        if not isinstance(failures, dict) or set(failures) != {"with_skill", "without_skill"}:
            return False
        if any(not isinstance(value, expected_type) or value for value in failures.values()):
            return False
    for arm in ("with_skill", "without_skill"):
        scores = agent.get(arm, {})
        if not isinstance(scores, dict) or not DIMENSIONS.issubset(scores):
            return False
...
def behavior_report_complete(reports: Path, count: int) -> bool:
    ...
    results = sorted({path.resolve() for path in reports.glob("*/*/result.json")})
    if len(results) != 1:
        return False
    if not results[0].is_relative_to(reports.resolve()):
        return False
    try:
...
def docker_ready(environment: dict[str, str] | None = None) -> bool:
    try:
...
def run(args: argparse.Namespace, environment: dict[str, str]) -> int:
    ...
    with (output / ".lock").open("a") as lock:
        ...
        state["prerequisites"] = {
            ...
            "docker_available": docker_ready(environment) if "3" in tiers else None,
        }
