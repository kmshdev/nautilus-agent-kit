...
merging. Native chunked catalog replay merges data-config iterators lazily,
unlike the non-chunked load path. Equal-`ts_init` groups can exceed requested
chunk size; query buffers and retained engine state also consume memory.
