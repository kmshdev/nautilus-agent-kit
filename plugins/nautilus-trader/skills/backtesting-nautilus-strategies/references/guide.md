...
canonical economic comparison. Native 0.64 chunked catalog replay lazily merges
data-config iterators; non-chunked loads and dynamic custom queries are separate
materializing paths. Equal-time groups, query buffers and engine state mean
`chunk_size` is not a hard RAM ceiling. Keep these conclusions in a short summary
...
At most one run config is supported despite the vector constructor; use one
node per run and a fresh process for each parameter-sweep run. With `chunk_size`
set, multiple catalog data configs are lazily merged with one lookahead per
stream; equal-time ties follow config order. Non-chunked runs load eagerly.
`chunk_size` is not a hard memory cap: equal-time groups expand it, and backend
buffers and retained engine state add memory. Run bounds intersect data bounds.
