...
This standalone repository contains all six workflows. From its repository
root (two directories above this guide), install the local marketplace:
...
to find this guide and the examples. Only this plugin is installed; this checkout
does not replace the original `kmshdev` marketplace. Start a new Codex thread after
...
```toml
[plugins."nautilus-trader@nautilus-trader"]
enabled = true
```
...
- [Run storage](examples/run-storage/README.md): an explicit native writer on
  ...
  and verifies DataFusion readback. NautilusTrader 0.64 rejects automatic live streaming
...
resources. The evidence targets upstream NautilusTrader 0.64.0 at commit
